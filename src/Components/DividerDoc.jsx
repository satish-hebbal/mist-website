...
function DividerDoc({ isDarkMode }) {
  ...
  return (
    <div className={`flex gap-4 ring-1 ml-8 h-screen p-4 rounded-md ${isDarkMode ? 'ring-zinc-700 ring-opacity-35' : 'ring-violet-200'}`}>
      <div className='flex flex-col gap-4 flex-grow overflow-y-auto pr-4 scrollbar-hide'>
        <h2 className='text-3xl font-bold text-zinc-400'>Divider</h2>
        <p className={textColor}>The Divider component is used to separate content in your layouts. It supports both horizontal and vertical orientations and adapts to light and dark modes.</p>
            ...
            {activeSection === 1 ? (
              <div className={`backdrop-blur-md bg-opacity-60 border rounded-md w-full ${isDarkMode ? 'bg-zinc-900 border-zinc-700 border-opacity-35' : 'bg-white border-violet-200'}`}>
                ...
                <div className='p-10 space-y-8'>
                  <div>
                    <h3 className={`text-lg font-semibold mb-4 ${headingColor}`}>Horizontal Divider</h3>
                  ...
                  <div>
                    <h3 className={`text-lg font-semibold mb-4 ${headingColor}`}>Vertical Divider</h3>
                    <div className={`flex items-center h-20 ${textColor}`}>
                      <p>Left of divider</p>
                      <Divider orientation="vertical" darkMode={isDarkMode} />
                      <p>Right of divider</p>
                    </div>
                  </div>
                  <div>
                    <h3 className={`text-lg font-semibold mb-4 ${headingColor}`}>Custom Styling</h3>
                    <div className={`${textColor}`}>
                      <p>Above the custom divider</p>
                      <Divider className="my-4 opacity-50" darkMode={isDarkMode} />
                      <p>Below the custom divider</p>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className={`w-full rounded-md overflow-auto ${codeBlockBg}`}>
                <CodeBlock 
                  code={`import { Divider } from 'mist-ui-comp';
      ...
      <div className='min-w-64 flex flex-col overflow-y-auto'>
        ...
        <div className='text-sm my-6 pl-2 ' >
          <dl className='flex flex-col gap-1'>
            <dt className={`font-semibold ${isDarkMode ? 'text-violet-400' : 'text-violet-700'}`}>orientation</dt>
            <dd className='font-mono'>· 'horizontal' | 'vertical'</dd>
