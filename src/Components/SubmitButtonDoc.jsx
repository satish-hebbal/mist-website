...
import { Divider, Code, Chip, SubmitButton } from 'mist-ui-comp';
import CodeBlock from '../CodeBlock';

function SubmitButtonDoc({ isDarkMode }) {
  const [activeSection, setActiveSection] = useState(1);

  const onClickProp = `onClick={handleSubmit}`;
  const labelProp = `label="Submit"`;
  ...
  return (
    <div className={`flex gap-4 ring-1 ml-8 h-screen p-4 rounded-md ${isDarkMode ? 'ring-zinc-700 ring-opacity-35' : 'ring-violet-200'}`}>
      <div className='flex flex-col gap-4 flex-grow overflow-y-auto pr-4 scrollbar-hide'>
        <h2 className='text-3xl font-bold text-zinc-400'>SubmitButton</h2>
        <p className={textColor}>The SubmitButton component provides a customizable button for form submissions or any action that requires user confirmation. It features a gradient background, hover and focus states, and supports dark mode.</p>
        ...
        <div className='flex flex-col'>
          ...
          <div className="w-full flex justify-center items-center h-64 overflow-hidden">
            {activeSection === 1 ? (
              <div className={`backdrop-blur-md bg-opacity-60 w-64 border rounded-md ${isDarkMode ? 'bg-zinc-900 border-zinc-700 border-opacity-35' : 'bg-white border-violet-200'}`}>
                <div className={`flex justify-between items-center border-b ${isDarkMode ? 'border-zinc-700 border-opacity-35' : 'border-violet-200'}`}>
                  <div className='flex items-center'>
                    <p className='p-4'>Submit button</p>
                  </div>
              ...
              <div className={`w-full h-64 rounded-md overflow-auto ${codeBlockBg}`}>
                <CodeBlock 
                  code={`import { SubmitButton } from 'mist-ui-comp';
      ...
      <div className='min-w-64 flex flex-col overflow-y-auto'>
        ...
        <div className='text-sm my-6 pl-2 ' >
          <dl className='flex flex-col gap-1'>
            <dt className={`font-semibold ${isDarkMode ? 'text-violet-400' : 'text-violet-700'}`}>onClick</dt>
            <dd className='font-mono'>· {onClickProp}</dd>
