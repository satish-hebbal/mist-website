...
import { Divider, Code, Chip, SelectOption } from 'mist-ui-comp';
import CodeBlock from '../CodeBlock';

function SelectOptionDoc({ isDarkMode }) {
  const [activeSection, setActiveSection] = useState(1);
  const [selectedOption, setSelectedOption] = useState('');

  const optionsProp = `options={['Option 1', 'Option 2', 'Option 3']}`;
  const valueProp = `value={selectedOption}`;
  const onChangeProp = `onChange={handleChange}`;
  const placeholderProp = `placeholder="Select an option"`;
  ...
  return (
    <div className={`flex gap-4 ring-1 ml-8 h-screen p-4 rounded-md ${isDarkMode ? 'ring-zinc-700 ring-opacity-35' : 'ring-violet-200'}`}>
      <div className='flex flex-col gap-4 flex-grow overflow-y-auto pr-4 scrollbar-hide'>
        <h2 className='text-3xl font-bold text-zinc-400'>SelectOption</h2>
        <p className={textColor}>The SelectOption component provides a dropdown menu for selecting a single option from a list. It includes customizable options, placeholder text, and a dark mode feature.</p>
        ...
        <div className='flex flex-col'>
          ...
          <div className="w-full flex justify-center items-center h-96 overflow-hidden">
            {activeSection === 1 ? (
              <div className={`backdrop-blur-md bg-opacity-60 mb-16 border rounded-md ${isDarkMode ? 'bg-zinc-900 border-zinc-700 border-opacity-35' : 'bg-white border-violet-200'}`}>
                <div className={`flex justify-between items-center border-b ${isDarkMode ? 'border-zinc-700 border-opacity-35' : 'border-violet-200'}`}>
                  <div className='flex items-center'>
                    <p className='p-4'>Select option</p>
                  </div>
              ...
              <div className={`w-full h-96 rounded-md overflow-auto ${codeBlockBg}`}>
                <CodeBlock 
                  code={`import { SelectOption } from 'mist-ui-comp';
...
export default SelectOptionDoc;