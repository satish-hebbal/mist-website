...
import { Divider, Code, Progress } from 'mist-ui-comp';
import CodeBlock from '../CodeBlock';

function ProgressDoc({ isDarkMode }) {
  ...
  return (
    <div className={`flex gap-4 ring-1 ml-8 h-screen p-4 rounded-md ${isDarkMode ? 'ring-zinc-700 ring-opacity-35' : 'ring-violet-200'}`}>
      <div className='flex flex-col gap-4 flex-grow overflow-y-auto pr-4 scrollbar-hide'>
        <h2 className='text-3xl font-bold text-zinc-400'>Progress</h2>
        
        <p className={textColor}>The Progress component is used to show the progress of an operation or to represent a value within a range. It supports various sizes, colors, and styles, including an animated loading state.</p>
          ...
          <div className="w-full flex justify-center items-center overflow-hidden">
            {activeSection === 1 ? (
              ...
              <div className={`w-full rounded-md overflow-auto ${codeBlockBg}`}>
                <CodeBlock 
                  code={`import { Progress } from 'mist-ui-comp';

function MyComponent() {
  return (
    <>
      {/* Size example */}
      <Progress size="md" value={75} showValueLabel label="Size: md" />

      {/* Color example */}
      <Progress color="primary" value={75} showValueLabel label="Color: primary" />

      {/* Thickness example */}
      <Progress thickness="medium" value={75} showValueLabel label="Thickness: medium" />

      {/* Loading state */}
      <Progress isLoading size="lg" label="Loading..." />
